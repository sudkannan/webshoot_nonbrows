...
stringfile_open (void *buf, size_t len, const char *mode)
{
  ...
  c = (stringfile_t *) allocate_mem (sizeof (stringfile_t));
  if (c == NULL)
   return NULL;
  ...
  if (c->mybuffer)
    {
      c->buffer = (char *) allocate_mem (len);
      if (c->buffer == NULL)
	{
	  nv_free (c);
	  return NULL;
...
arrayfile_t * arrayfile_fopen(const char *name, const char *mode) {
  arrayfile_t *file = (arrayfile_t *)allocate_mem(sizeof(arrayfile_t));
  if (!file) return NULL;
  ...
  printf("before arrayfile_fopen \n");

  file->array = (char **)allocate_mem(kArrayfileDefaultAlloc * sizeof(void *));
  if (!file->array) goto array_fail;
  file->array_size = kArrayfileDefaultAlloc;
  file->array_lens = allocate_mem(file->array_size * sizeof(int));
  if (!file->array_lens) goto lens_fail;
  file->array_allocs = allocate_mem(file->array_size * sizeof(int));
...
/*ssize_t arrayfile_fwrite(const char *b, size_t s, size_t n, arrayfile_t *file) {
  ...
  if (count == file->array_size) {
    if (file->keep_output) {
      // grow the capacity 
	  fprintf(stderr,"before allocate_mem \n");	
      temp = allocate_mem(file->array_size * sizeof(void *) * 2);
	  fprintf(stderr,"after allocate_mem \n");
      memcpy(temp, file->array, count);
      file->array = (char **)temp;
      if (!file->array) return 0;

      temp_int = (int *)allocate_mem(file->array_size * sizeof(int) * 2);

      fprintf(stdout, "%d %d %d %d\n",file->array_lens[idx], count, file->array_size, prev_array_lens_size);

	   while(idx < count) {	
			temp_int[idx] = file->array_lens[idx];
			idx++;
		}
     
        prev_array_lens_size = file->array_size * sizeof(int) * 2;

      /*if(file->array_lens && count ){
		  fprintf(stderr,"count %d \n",count);
	      memcpy(temp, file->array_lens, (count/2)* sizeof(int));
         
    	}*/

  /*    file->array_lens = (int *)temp_int;

      if (!file->array_lens) return 0;    

      temp = allocate_mem(file->array_size * sizeof(int) * 2);
      
     /*if(file->array_allocs && count ){
          fprintf(stderr,"count %d \n",count);
          memcpy(temp, file->array_allocs, (count)* sizeof(int));

        }*/

    /*  file->array_allocs = (int *)temp;

      if (!file->array_lens || !file->array_allocs) return 0;
      // initialize the newly-allocated upper halves
      //memset(file->array, 0, count * sizeof(void *));
      //memset(file->array_lens, 0, count * sizeof(int));
      //memset(file->array_allocs, 0, count * sizeof(int));
...
ssize_t arrayfile_fwrite(const char *b, size_t s, size_t n, arrayfile_t *file) {
  ...
  if (count == file->array_size) {
    if (file->keep_output) {
      ...
      temp = (char **)allocate_mem( file->array_size * sizeof(void *) * 2);
      memcpy(temp, file->array, file->array_size * sizeof(void *) );
      file->array = (char **)temp;

      //file->array = (char **)realloc(file->array, file->array_size * sizeof(void *) * 2);
      if (!file->array) return 0;
      ...
      temp = (char **)allocate_mem( file->array_size * sizeof(int) * 2);
      memcpy(temp, file->array_lens, file->array_size * sizeof(int) );
      file->array_lens = (int *)temp;
      ...
      temp = (char **)allocate_mem( file->array_size * sizeof(int) * 2);
      memcpy(temp, file->array_allocs, file->array_size * sizeof(int) );
      file->array_allocs= (int *)temp;
...
ssize_t arrayfile_fwrite1(const char *b, size_t s, size_t n, arrayfile_t *file) {
  char *buf;
  size_t size = s * n;
  int count;
  if (!file) return 0;
...
char * arrayfile_join(arrayfile_t *file, int *len) {
  ...
  joinbuf = allocate_mem(total_size + 1);
...
int arrayfile_fclose(arrayfile_t *file) {
  ...
  for (i = 0; i < file->array_size; i++) {
    nv_free(file->array[i]);
    file->array[i] = NULL;
  }
  nv_free(file->array);
  nv_free(file->array_lens);
  nv_free(file->array_allocs);
  nv_free(file);
  return 0;
}
