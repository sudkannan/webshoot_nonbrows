...
void ClearBenchmarks() {
  for (int i = 0; i < benchmark_count; i++) nv_free(bench_info_list[i].name);
  benchmark_count = 0;
}
...
static int RunOne(bench_info *bench, run_data *data) {
  ...
  if (run_model == kRunModelRepeated) {
    for (data->runs = 0; data->elapsed < kMinBenchRuntime || data->runs < REPEATED_RUNS;
         data->runs++) {
...
static void PrintScores() {
  int i;
  for (i = 0; i < benchmark_count; i++) {
    printf("Benchmark %s: usec %d, iters %d, usec/run %d score %.2f runs %d\n", 
...
//  RegisterBenchmark(strdup("Spectralnorm"), run_spectralnorm, 350, 150020779,
 ...
 // RegisterBenchmark(strdup("Fasta"), run_fasta, 10000, 51667385, NULL, NULL);
  ...
  RegisterBenchmark(strdup("Fasta"), run_fasta, 300000, 51667385, NULL, NULL);
...
//  RegisterBenchmark(strdup("Pidigits"), run_pidigits, 5000, 406976744,
  //                  NULL, NULL);
   ...
   //RegisterBenchmark(strdup("Fasta"), run_fasta, 30000000, 51667385, NULL, NULL);	
   //RegisterBenchmark(strdup("Revcomp"), run_revcomp, 0, 23542857, NULL, NULL);
   //RegisterBenchmark(strdup("Knucleotide"), run_knucleotide, 0, 433893130,NULL, NULL);
