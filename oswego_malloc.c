...
void *use_nvmap(size_t s, struct rqst_struct *);
...
#define dlfree                 free*/
#define dlnvmalloc               nv_malloc
#define dlnvcalloc               nv_calloc
...
DLMALLOC_EXPORT void* dlnvmalloc(size_t);
...
DLMALLOC_EXPORT void* dlnvcalloc(size_t, size_t);
...
int setup_nvmalloc_mapfile(char *filepath, unsigned long bytes) {
    int result;
    int fd = -1;
...
unsigned long mmap_startaddr = 0;
unsigned int proc_id = 1999;
size_t nvoffset = 0;
void *nvmap = NULL;

void *use_nvmap(size_t s, struct rqst_struct *rqst){
...
#endif
	 {
        ...
        if( nvmap == NULL) {
			...
			if(rqst) {
				...
				//get the total mmap blocks of process
				chunk_id = get_proc_num_maps(rqst->pid);				
		
				//increment it by 1;
				//by this way we will keep on incrementing
			...
			a.pflags = 1;
			  ...
			  mmap_startaddr = (unsigned long)nvmap;
 			  fprintf(stderr,"use_nvmap mmap_startaddr %lu ", mmap_startaddr);
		}
		else {
			ptr	= nvmap + nvoffset;
		}
...
int set_mmap_start_addr(struct rqst_struct *rqst){

	if(!rqst)
		return -1;
	rqst->mmap_straddr = mmap_startaddr;

	return 0;
}

int set_mmap_id(struct rqst_struct *rqst){
...
int update_nvram_mdata(struct rqst_struct *rqst, void* mem){
    ...
    if(set_mmap_id(rqst)) {
    	fprintf(stderr,"failed updating mmap id");
    	return -1;
    }
    if(set_mmap_start_addr(rqst)) {
    	fprintf(stderr,"failed updating mmap start address");
    	return -1;
    }
...
static void* sys_alloc(mstate m, size_t nb, struct rqst_struct *rqst) {
//NVRAM changes
//static void* sys_alloc(mstate m, size_t nb, int val, struct rqst_struct *rqst)
  char* tbase = CMFAIL;
  size_t tsize = 0;
  flag_t mmap_flag = 0;
  ...
  /*if (use_mmap(m) && nb >= mparams.mmap_threshold && m->topsize != 0) {
	...
	fprintf(stderr, "in use_mmap(m) && nb >= mparams.mmap_threshold \n");
 ...
 //NVRAM Changes
  ...
  else if (MORECORE_CONTIGUOUS && !use_noncontiguous(m) && tbase == CMFAIL ) {
    char* br = CMFAIL;
...
void* dlpnvread(size_t bytes, struct rqst_struct *rqst) {


	return nv_map_read(rqst, NULL);
...
void* dlpnvmalloc(size_t bytes, struct rqst_struct *rqst) {
	...
	//process object structure if they do not exist
	if(nv_mmap(rqst)) {
		fprintf(stdout, "updating process object failed");
	}
...
//void* dlmalloc(size_t bytes) {
 ...
 //NVRAM Changes
    if(update_nvram_mdata(rqst, mem)){
    	fprintf(stderr,"updating nvram metadata failed");
    	return NULL;
    }
    //NVRAM Changes
...
void* dlnvcalloc(size_t n_elements, size_t elem_size) {
 ...
 // }
  ...
  mem = dlnvmalloc(n_elements * elem_size);
...
static mchunkptr try_realloc_chunk(mstate m, mchunkptr p, size_t nb,
  ...
  fprintf(stderr,"callomg try_realloc_chunk \n");

  size_t oldsize = chunksize(p);
  fprintf(stderr,"oldsize %zu \n", oldsize);
  mchunkptr next = chunk_plus_offset(p, oldsize);
  if (RTCHECK(ok_address(m, p) && ok_inuse(p) &&
              ok_next(p, next) && ok_pinuse(next))) {

    fprintf(stderr,"entering RTCHECK \n");

    if (is_mmapped(p)) {

      fprintf(stderr,"is mmaped \n");
...
//NVRAM Changes
//nvmem realloc
//commented code which free data like dlfree and internal_free
void* dlnvrealloc(void* oldmem, size_t bytes) {
  void* mem = 0;
  if (oldmem == 0) {
    mem = dlnvmalloc(bytes);
  }
  else if (bytes >= MAX_REQUEST) {
    MALLOC_FAILURE_ACTION;
  }
