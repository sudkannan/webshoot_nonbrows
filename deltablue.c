...
List List_Create(int initialCount)
{
  ...
  newList = (List) allocate_mem(sizeof(ListStruct));
  if (newList == NULL) Error("out of memory");
  newList->slots = (Element *) allocate_mem(initialCount * sizeof(Element));
...
{
  if (list->slots == NULL) Error("bad ListStruct; already nv_freed?");
  nv_free(list->slots);
  list->slots = NULL;
  list->slotCount = 0;
  list->first = 0;
...
void Grow(List list)
{
  ...
  //count  = list->slotCount * sizeof(Element);

  list->slotCount += min(max(list->slotCount, 2), 512);

  //temp = allocate_mem(list->slotCount);
	
   //memcpy(temp,(void *)list->slots,count);	

   //list->slots = temp;	

  list->slots = malloc((list->slotCount * sizeof(Element)));
...
Variable Variable_Create(char *name, long initialValue)
{
  ...
  new = (Variable) allocate_mem(sizeof(VariableStruct));
...
void Variable_Destroy(Variable v)
{
  if (v->constraints == NULL) {
    Error("bad VariableStruct; already nv_freed?");
  }
...
Constraint Constraint_Create(int variableCount, Strength strength)
{
  ...
  new = (Constraint) allocate_mem(sizeof(ConstraintStruct)
...
{
  if (c->execute == NULL) {
    Error("bad ConstraintStruct; already nv_freed?");
  }
  c->execute = NULL;
  nv_free(c);
}
