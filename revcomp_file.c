...
//we dont have file but nvram pointer
void *nv_outptr = NULL;
...
void persistent_write(int fd, const void *ptr, size_t size) {
#ifdef ARRAYFILE
...
revcomp_main(void) {
    ...
    char *mybuffer = calloc(stdin_size + LINE_LENGTH, 1);
    ...
    _fread(mybuffer,1,stdin_size, input_file);

   //while (_fgets((char *)current, end - current, input_file)) {

   while((mybuffer = readline((char *)current, end - current,mybuffer)) != NULL) {
