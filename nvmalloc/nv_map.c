...
static struct chunk* create_chunk_obj(struct rqst_struct *rqst, unsigned long curr_offset, struct proc_obj* proc_obj) {
	...
	chunk->mmap_straddr = rqst->mmap_straddr;
...
#ifdef NV_DEBUG
	fprintf(stderr, "Setting offset chunk->vma_id %u to %lu \n",chunk->vma_id, chunk->offset);
#endif
...
/*static int set_process_chunk(struct chunk *chunk, int proc_id) {
...
/*static int get_process_id(struct chunk *chunk) {
...
#ifdef NV_DEBUG
       fprintf(stderr,"update_offset calling \n");
#endif
       //if application has supplied request id, neglect
       if(!rqst->id) {     
    	   if(rqst->var){ 
...
void *map_process(struct rqst_struct *rqst) {
  ...
  nvarg.ref_count = 1;
   ...
   nvmap  = (char *)syscall(__NR_nv_mmap_pgoff,0 ,NVRAM_DATASZ,  PROT_READ | PROT_WRITE, MAP_PRIVATE, &nvarg);
   if (nvmap == MAP_FAILED) {
       close(fd);
       goto error;
   }
...
#ifdef NV_DEBUG
    	    fprintf(stderr,"first time read by process\n");
        	fprintf(stderr, "finished reading map file\n");
#endif
	        //Since native client code maps to the file by itself
    	    //we dont call the map process. Instead use already
        	//passed address and set request.mem field
	        //map_read =  map_process(rqst);
    	      map_read = (void *)map;
		 }else {
#ifdef NV_DEBUG
    	    fprintf(stderr, "map_read not null \n");
#endif
	 	 }
         map_read = (void *)map;*/
    //find the chunk   
...
int incr_outof_core_lock(void){
  ...
  if(!l_queue){
    printf("disable_outof_core_access: l_queue is null \n");
    return -1;
  }
  l_queue->outofcore_lock++;
  return 0;
}
...
int decr_outof_core_lock(void){
  ...
  //This functions should be atomic
  l_queue->outofcore_lock--;
  return 0;
...
int is_outof_core_lock_disabled(void){
 ...
 if(!l_queue){
   ...
   //unless the outofcore_lock is 0
   //we will keep returning -1, failure
  if(l_queue->outofcore_lock)
		return -1;
  else
		return 0;
...
int print_outof_core_lock(void){
 ...
 if(!l_queue){
  ...
  fprintf(stderr,"outofcore_lock %d \n",l_queue->outofcore_lock);
 return 0;
}
